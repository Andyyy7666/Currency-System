...
local currentCash = exports.Money_Script:CheckWallet()
	...
	while true do
		...
		if pumpDistance < 2.5 then
			...
			if Config.UseMoney then
				currentCash = exports.Money_Script:CheckBank()
				end
		else
			isNearPump = false
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	if pumpObject then
		exports.Money_Script:RemoveCash(currentCost)
	end
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								exports.Money_Script:RemoveCash(Config.JerryCanCost)

								currentCash = exports.Money_Script:CheckWallet()
							end
						else
							if Config.UseMoney then
								...
								if refillCost > 0 then
									if currentCash >= refillCost then
										...
										if IsControlJustReleased(0, 38) then
											exports.Money_Script:RemoveCash(refillCost)
